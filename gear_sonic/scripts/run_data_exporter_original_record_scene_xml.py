...
DEFAULT_SCENE_XML_PATH = "outputs/latest_random_plate_cube_scene.xml"
...
class SonicDataExporterConfig:
    ...
    record_scene_xml: bool = True
    """Copy the scene XML into the dataset when each episode starts recording."""

    scene_xml_path: str = DEFAULT_SCENE_XML_PATH
    """Scene XML path to copy for each saved episode."""
...
class GrootDataCollector:
    ...
    def __init__(
        ...
        self.scene_xml_path = _resolve_scene_xml_path(scene_xml_path)
        self.record_scene_xml = record_scene_xml
        self._scene_xml_recorded_episode_indices: set[int] = set()
    ...
    def _check_recording_commands(self):
        ...
        if key == "c":
            ...
            if self._episode_state.get_state() == self._episode_state.RECORDING:
                self._initial_yaw = None
                self._record_scene_xml_for_current_episode()
                self._print_and_say(
    ...
    def _record_scene_xml_for_current_episode(self) -> None:
        if not self.record_scene_xml:
            return
        if self.scene_xml_path is None:
            return
        episode_index = int(self.data_exporter.episode_buffer["episode_index"])
        if episode_index in self._scene_xml_recorded_episode_indices:
            return
        if not self.scene_xml_path.exists():
            self._print_and_say(
                f"Scene XML not found, skipping copy: {self.scene_xml_path}",
                say=False,
                blocking=False,
            )
            return

        scene_dir = Path(self.data_exporter.meta.root) / "scene_xml"
        scene_dir.mkdir(parents=True, exist_ok=True)
        dst = scene_dir / f"episode_{episode_index:06d}.xml"
        shutil.copy2(self.scene_xml_path, dst)
        self._scene_xml_recorded_episode_indices.add(episode_index)
        print(f"[SceneXML] Saved scene XML for episode {episode_index}: {dst}")
...
def _resolve_scene_xml_path(scene_xml_path: str | Path | None) -> Path | None:
    if scene_xml_path is None:
        return None
    path = Path(scene_xml_path).expanduser()
    if path.is_absolute():
        return path
    ...
    data_exporter = Gr00tDataExporter.create(
        ...
        script_config={
            **robot_config,
            ...
            "record_scene_xml": config.record_scene_xml,
            "scene_xml_path": str(_resolve_scene_xml_path(config.scene_xml_path)),
        },
    )
